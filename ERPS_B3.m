...
chMap=ecog_grid;
tim = cumsum([length(idx1) length(idx2) length(idx3) length(idx4)]);
mvmts = {'Rt Thumb','Leg','Left Thumb','Head','Lips','Tongue','Squeeze Both Hands'};
...
for iter=1:length(D)
    ...
    sgtitle(mvmts{iter});
    filename = fullfile('F:\DATA\ecog data\ECoG BCI\Results\B3 results\ERPs Arrow\beta',mvmts{iter});
    saveas(gcf,filename)
