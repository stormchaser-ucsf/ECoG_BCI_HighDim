function [acc_lstm_sample,lstm_output] = ...
    get_lstm_performance_multistate(filepath,net_bilstm,Params,lpFilt,folders_idx)



% get the name of the files
D = dir(filepath);
folders_idx= folders_idx+2;
files=[];
for j=1:length(folders_idx)
    tmp_path = fullfile(filepath,D(folders_idx(j)).name,'Imagined');
    files = [files;findfiles('.mat',tmp_path)'];
end
