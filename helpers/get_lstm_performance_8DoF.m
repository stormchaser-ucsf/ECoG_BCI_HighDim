function [acc_lstm_sample,acc_mlp_sample,acc_lstm_trial,acc_mlp_trial] = ...
    get_lstm_performance_8DoF(files,net_bilstm,Params,lpFilt,num_targets)
...
for i=1:length(files)
    ...
    if files_loaded
        ...
        for j=1:length(raw_data)
            ...
            % store results
            if TrialData.TargetID <= num_targets
                if state_idx(j)==3
...
acc_mlp=zeros(num_targets+1,num_targets+1);
acc_mlp_trial=zeros(num_targets+1,num_targets+1);
for i=1:length(files)
    %disp(i)
    ...
    if files_loaded
        ...
        %if tid<=num_targets
        acc1=zeros(num_targets+1,num_targets+1);
